...
| `finops/orphan-cleanup` | Resource-matching logic (tag + name-prefix), and that `--delete` actually removes what was found — against `moto` (mocked AWS), not a real account | `pytest finops/orphan-cleanup` | `validate.yml` → `finops` |
| `finops/cost-dashboards` | Report formatting/sorting/rounding logic, against fixed Cost Explorer response shapes (the AWS call itself isn't unit-tested — see the file's own docstring for why) | `pytest finops/cost-dashboards` | `validate.yml` → `finops` |
| `ai-ops-assistant` | Runbook lookup and recent-commit gathering against this repo's real files/git history, prompt formatting, Prometheus response parsing (mocked HTTP), and Alertmanager webhook payload parsing against a fixture matching Alertmanager's real webhook JSON shape — the Anthropic API call itself isn't unit-tested, same boundary as the AWS calls above | `pytest ai-ops-assistant` | `validate.yml` → `finops` |
