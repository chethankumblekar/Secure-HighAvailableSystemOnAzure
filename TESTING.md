...
| `ai-ops-assistant` | Runbook lookup and recent-commit gathering against this repo's real files/git history, prompt formatting, and Prometheus response parsing (mocked HTTP) — the Anthropic API call itself isn't unit-tested, same boundary as the AWS calls above | `pytest ai-ops-assistant` | `validate.yml` → `finops` |
