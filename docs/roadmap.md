...
| 3 — CI/CD + GitOps | GitHub Actions (SAST/scan/SBOM/sign) → ArgoCD auto-deploy | 🚧 In progress | `ci.yml` (build/test → Trivy → Syft → GHCR push → cosign sign+attest, see `docs/adr/0003`), `codeql.yml`, `platform/argocd` app-of-apps written and offline-verified (actionlint clean); not yet pushed/run for real, ArgoCD sync not yet proven end-to-end |
